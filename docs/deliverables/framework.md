Of course. Here are the complete, finalized documents based on our entire conversation.

You can copy the full text from each section and paste it into a separate file (`PRD_FlatFinder_Web2_MVP.md` and `Architecture_FlatFinder_Web2_MVP.md`) in your `docs` folder.
...
### **Document 2: Fullstack Architecture Document**
...
graph TD
    ...
    subgraph Backend Services (GCP/Firebase)
        ...
        API -- Data I/O --> DB[(🔥 Firebase Firestore)]
        Scraper[🕷️ Scraping Agent (Cloud Function)] -- Raw Data --> API
        API -- Verification --> LandRegistry[🏛️ Land Registry APIs]
        API -- Geocoding --> GoogleMaps[📍 Google Maps API]
    end

    subgraph External & Automation
        Scraper --> Craigslist & Kijiji
        WebApp -- Fiat Payments --> Stripe[💳 Stripe]
        API -- Webhook --> Marketing[📢 Marketing Service (n8n/Other)]
    end
````
...
| :--- | :--- | :--- |
| **Cloud Platform** | Google Cloud Platform (GCP) | Selected to utilize user credits. |
| **Backend Services**| Firebase | Firestore DB, Cloud Functions for API/agents, Hosting. |
| **Frontend** | Next.js / React | Modern, performant framework for the user-facing app. |
| **Authentication** | Clerk | Handles all user sign-up, login, and session management. |
| **Payments** | Stripe (Fiat), WalletConnect (Crypto) | Securely processes all payments off-platform. |
...
| **UI Design**| Webflow | Founder will provide designs from Webflow as the visual source of truth. |
| **UI Library** | Custom | No pre-built UI libraries like shadcn/ui. |
| **Marketing Automation**| External Service (e.g., n8n) | Decoupled via webhooks for email campaigns and notifications. |
| **Verification** | Alchemy (KYC), OnLand (ON), LTSA (BC) | For landlord identity and property ownership verification. |

### 2\. Frontend Architecture

  - **Framework:** Next.js with App Router.
  - **Styling:** Tailwind CSS will be used to implement the custom designs provided via Webflow.
  - **State Management:** React Context API for simple global state (e.g., user session), Zustand for more complex client-side state.
  - **Data Fetching:** React Query (TanStack Query) for fetching, caching, and updating server data.

### 3\. Backend Architecture

  - **Platform:** Firebase Cloud Functions (Node.js/TypeScript).
  - **Database:** Firestore in Native mode for flexible, scalable data storage.
  - **API:** A single API gateway entry point routing to different functions for modularity (e.g., `user`, `listings`, `payments`).
  - **Asynchronous Agents:**
      - **Scraping Agent:** A scheduled Cloud Function that runs periodically.
      - **Vetting Agent:** A Cloud Function triggered when new data is added to a temporary collection.
      - **Outreach Agent:** A Cloud Function triggered by a user action in the app.
...
### 4\. Code & Repository Structure
  ...
  - **Structure:**
      ...
      - `/apps/api`: The Firebase Functions backend code.
      - `/packages/shared`: Shared code, primarily TypeScript types for API requests/responses to ensure consistency.
      - `/docs`: Contains this document, the PRD, and other planning materials.
